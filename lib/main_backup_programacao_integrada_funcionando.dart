...
class HousekeepingEhsChecklistScreen extends StatefulWidget {
  ...
  final VoidCallback? aoFinalizarProgramacao;
...
widget.aoFinalizarProgramacao?.call();
...
class _VisualizarInspecoesProgramadasScreenState
  ...
  void removerProgramacaoRealizada(int index) {
    setState(() {
      if (index >= 0 && index < programacoesInspecao.length) {
        programacoesInspecao.removeAt(index);
      }
    });
  }

  void realizarInspecaoProgramada(
    BuildContext context,
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => HousekeepingEhsChecklistScreen(
          responsavelInicial: programacao.responsavel,
          areaInicial: programacao.area,
          aoFinalizarProgramacao: () {
            removerProgramacaoRealizada(index);
          },
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: programacoesInspecao.isEmpty
          ? const Center(
              child: Padding(
...
class _ProgramarInspecaoScreenState extends State<ProgramarInspecaoScreen> {
  ...
  void salvarProgramacao() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Inspeção programada com sucesso.'),
      ),
    );
